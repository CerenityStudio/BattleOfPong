...
public class Ball : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if(col.gameObject.tag == "Goal 1")
        {
            ...
            GameManager.instance.SpawnBall();
            Destroy(gameObject);
            if (GameManager.instance.goldenGoal) //Fungsi ini digunakan untuk mevalidasi kondisi sedang dalam mode Golden Goal
            {
    ...
    {
        isBounce = true;
        yield return new WaitForSeconds(2f); //Kalau menggunakan ienumerator, sebagai returnnya harus menggunakan yield
        isBounce = false;
    }
}
